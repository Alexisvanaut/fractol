...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/06/27 15:43:55 by alvanaut          #+#    #+#             */
/*   Updated: 2025/06/28 13:13:08 by alvanaut         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "../includes/fractol.h"

static int	myfract_iter(double c_re, double c_im, int max_iter)
{
	double	z_re;
	double	z_im;
	double	tmp;
	...
	while (y < HEIGHT)
	{
		...
		while (x < WIDTH)
		{
			...
			c_im = data->min.im + (double)y / HEIGHT * (data->max.im
					- data->min.im);
			iter = myfract_iter(c_re, c_im, data->max_iter);
